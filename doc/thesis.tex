...
% TODO:
% * devo includere delle metriche sul codice e.g. il numero di linee di codice 1300
% * devo dire quanti MB al secondo sono processati
% * devo parlare più approfonditamente di ARRAY
% * devo parlare di cos'è POSIX
...
\includegraphics[width=\linewidth]{er_diagram.pdf}
% \begin{picture}(6,6)
...
just tells OpenMP to parallelize this for across the default number of cores
...
them, the third just says that those variables are shared among threads. Finally
\verb+reduction(|: has_transmitted_fire)+ just tells that at the end of the
execution each thread all the private copies of \verb+has_transmitted_fire+ shall
be ORed together. This is done to efficiently implement the halting condition
avoiding problems like lock contention or false sharing.
