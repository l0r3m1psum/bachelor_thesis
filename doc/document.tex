...
\marginpar{Nel paragrafo \ref{sec:statics} tutti questi dati sono descritti con
più precisione.}
...
\marginpar{Sempre sulla via del \eng{model-checking} penso sia meglio (per come
è scritto il modello) non considerare celle rettangolari perché le formule non
tengono conto della dimensione di queste ultime e ovviamente è più probabile
che un icendio si trasmetta per il lato lungo di un rettangolo che per quello
corto.}
...
cella limitrofa $(i+\e_1, j+\e_2)$ alla cella corrente $(i, j)$.
...
H_{ij} = \cases{1 + G_{ij}, &se $0 \leq H_{ij} \leq 2$;\cr
                  0, &se $H_{ij} = 255$.}\label{eq:height}
\end{equation}
...
A_{ij} = \cases{U_{ij}/100, &se $0 \leq U_{ij} \leq 100$;\cr
                0, &se $U_{ij} = 255$.}\label{eq:inhabitants}
\end{equation}
...
W_{ij} = \cases{0, &se $W1_{ij} = 0$;\cr
                ...
                0.5, &se $W1_{ij} = 4$;\cr
                1, &se $W1_{ij} = 253$;\cr
                1, &se $W1_{ij} = 255$.}\label{eq:water}
\end{equation}
...
\begin{equation}
p_{ij}(\e_1, \e_2, t) = k_0 S_{ij} C_{i+\e_1j+\e_2}(t) d(\e_1, \e_2) f_w f_P\textrm{,}\label{eq:prob}
\end{equation}

dove $k_0$ è un parametro di ottimizzazione di soglia e

\begin{equation}
C_{i+\e_1j+\e_2}(t) = \sin\left(\pi\frac{B_{i+\e_1j+\e_2}(t)}{\gamma_{i+\e_1j+\e_2}}\right)\textrm{,}\label{eq:combust}
\end{equation}
...
\begin{tabular}{|c|l|}
	...
	\hline
	$B(t)$ & combustibile disponibile\\
	$N(t)$ & stato (in fiamme o meno)\\
	$p(\epsilon_1, \epsilon_2, t)$ & probabilità di trasmissione\\
	\hline
\end{tabular}
...
In fine sia $u_{ij}(t)$ l'\eng{input} esogeno che definisce o lo stato di quiete
o di incendio con i valori 0 e 1 rispettivamente, che all'atto pratico è
ottenuta attraverso dati sul campo. La dinamica del sistema è la seguente:
